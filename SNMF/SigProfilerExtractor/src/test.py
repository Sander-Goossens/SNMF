...
def main():
    ...
    for fold in folds:
        ...
        N_test = 1000
        test_path = "C:/Users/sande/PycharmProjects/MEP/data/processed/bootstrapped_sameSplit_sorted/N_{}".format(N_test)
        ...
        for k in range(min_sig,max_sig+1):
            ...
            for c_idx, lambda_c in enumerate(l_c):
                ...
                # TEST / VALIDATION
                    ...
                    # else:
                    #     results[c_idx, p_idx, 28:]= sig.test_sigProfilerExtractor("text", output_path, test_data, test_label, minimum_signatures=k, maximum_signatures=k, nmf_replicates=reps, lambda_c= float(lambda_c), lr= float(lr), lambda_p= float(lambda_p), filter = False)
